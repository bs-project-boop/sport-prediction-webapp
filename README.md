...
|-------|---------------|------------------|
| Timezone bug audit (MotoGP, FIBA, IBL) | 2026-07-26 | Audit found NO timezone-aware vs timezone-naive comparison bug in any of the 3 clients — all datetime comparisons use consistent `WIB` timezone (window_start via `datetime.combine(tzinfo=WIB)`, parsed datetimes via `parse_wib()`/`.astimezone(WIB)`). Scope is 3 clients (Grand Slam removed separately). Minor improvement: added `logging` + `logger.warning()` to 2 previously silent `except Exception: continue` sites in `sports_v31_espn_ingest.py` (lines ~194 and ~420) so parse failures are no longer invisible. All 9 engine scripts compile cleanly. |
